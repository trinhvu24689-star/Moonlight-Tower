...
import { Axe, ChefHat, User, Magnet, Sword, Crown, Diamond, Flame } from 'lucide-react';
...
const SPRITE_IDS = {
  ...
  HOUSE: 180, GRILL: 160, COUNTER: 165,
...
// --- BUILDING ENTITY (Thêm cái này để hiển thị Lò nướng và Quầy) ---
export const BuildingEntity: React.FC<{ x: number, y: number, type: 'grill' | 'counter' }> = ({ x, y, type }) => (
    <BaseEntity x={x} y={y} className="-ml-10 -mt-[60px]" scale={1.5}>
        <SpriteImage id={type === 'grill' ? SPRITE_IDS.GRILL : SPRITE_IDS.COUNTER} className="w-16 h-16" />
        {type === 'grill' && <div className="absolute top-0 animate-pulse text-red-500"><Flame size={20}/></div>}
    </BaseEntity>
);

// --- LOOT ENTITY ---
export const LootEntity: React.FC<{ x: number, y: number, type: 'meat' | 'gold' | 'ruby' }> = ({ x, y, type }) => {
    let icon = <SpriteImage id={SPRITE_IDS.LOOT_MEAT} className="w-8 h-8" />;
    if (type === 'gold') icon = <SpriteImage id={SPRITE_IDS.LOOT_GOLD} className="w-8 h-8" />;
    if (type === 'ruby') icon = <Diamond size={20} className="text-red-500 fill-red-300 animate-pulse" />;
...
// ... (Giữ nguyên các Entity cũ: Enemy, Wall, Tree, Customer, etc.)
export const HouseEntity: React.FC<{ x: number, y: number }> = ({ x, y }) => (<BaseEntity x={x} y={y} className="-ml-10 -mt-[80px]" scale={1.2}><SpriteImage id={SPRITE_IDS.HOUSE} className="w-16 h-16" /></BaseEntity>);
...
export const WarehouseEntity: React.FC<{ x: number, y: number }> = ({ x, y }) => (<BaseEntity x={x} y={y} className="-ml-16 -mt-[120px]" scale={1.2}><SpriteImage id={180} className="w-24 h-24" /></BaseEntity>);
